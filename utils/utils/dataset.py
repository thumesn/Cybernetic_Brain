...
class ColoredMNIST_cnc(datasets.VisionDataset):
    def __init__(self, root='', env='train1'):
        super(ColoredMNIST_cnc, self).__init__(root)
        ...
        for img, target in data_label_tuples:
            ...
            self.data_label_color_tuples.append(tuple([img, target]))
    ...
    """
        img, target= self.data_label_color_tuples[index]
        # img = transforms.ToTensor()(img)
